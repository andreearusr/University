...
public class Main {
    ...
    private static int[][] F; //matricea initiala + bordata,
    private static float[][] W = new float[m][n]; //matricea de filtrare
    private static float[][] V = new float[M][N]; //matricea rezultat
...
//        String type = reader.readLine();
        ...
        if (type.equals("seq")) {
            ...
            for (int i = 0; i < p; i++) {
                threads[i] = new MyThread(i, p, V, M, N);
                threads[i].start();
            }
