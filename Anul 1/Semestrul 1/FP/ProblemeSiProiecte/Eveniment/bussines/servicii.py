...
from sortare.sortari import GenericSort
class ServicePersoana(object):
    ...
    def modifica_persoana(self,persoana):
        ...
        self.__validator_persoana.valideaza_pers(persoana)
        self.__repo_persoana.modifica(persoana)
...
class ServiceInscriere(object):
    ...
    def get_inscrieri(self):
        ...
        inscrieri_dtos = self.get_inscrieri_recursiv(inscrieri)
...
#            inscrieri_dtos.append(inscriere_dto)
    ...
    def get_inscrieri_recursiv(self,inscrieri):
        #cazul de baza
        if inscrieri ==[]:
            return []
        ...
        #caz inductiv
            ...
            id_inscriere = inscrieri[0].get_id_inscriere()
            
            id_persoana = inscrieri[0].get_id_persoana()
            persoana = self.__repo_persoana.cauta_dupa_id(id_persoana)
            nume = persoana.get_nume()
            
            id_eveniment = inscrieri[0].get_id_eveniment()
            ev = self.__repo_eveniment.cauta_dupa_id(id_eveniment)
            descriere_even = ev.get_descriere_even()
            pret = inscrieri[0].get_pret()
            
            inscriere_dto = InscriereDTO(id_inscriere,nume,descriere_even,pret)

        return [inscriere_dto]+self.get_inscrieri_recursiv(inscrieri[1:])
...
#        lista.sort(key=lambda ev:(ev.get_descriere_even(),ev.get_data_eveniment())) 
        ...
        lista1 = GenericSort.sortare(self, lista, algoritm = "Selection Sort", key = lambda ev:(ev.get_descriere_even(),ev.get_data_eveniment()), rev = False) 
        return lista1
...
#        evenimente.sort(key=lambda ev:ev.get_nr_participanti(), reverse = True)
        evenimente = GenericSort.sortare(self, evenimente, algoritm = "Shell Sort", key = lambda ev:ev.get_nr_participanti(), rev = True) 
