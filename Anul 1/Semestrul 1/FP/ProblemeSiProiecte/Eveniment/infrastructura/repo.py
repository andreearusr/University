...
#        raise RepoError("Persoana inexistenta!\n")
        i=0
        persoana = self.cauta_dupa_id_recursiv(i, id_persoana)
        return persoana

    def cauta_dupa_id_recursiv(self,i,id_persoana):
        ...
        if i<len(self._elems):
            ...
            return self.cauta_dupa_id_recursiv(i+1, id_persoana)
        raise RepoError("Persoana inexistenta!\n")
    ...
    def modifica(self, persoana):
        ...
        for i in range(len(self._elems)):
            if self._elems[i]== persoana_noua:
                self._elems[i]= persoana_noua
                return
        '''
#        for el in self._elems:
#            if el.get_id_persoana()==persoana.get_id_persoana():
#                el.set_nume(persoana.get_nume())
#                el.set_adresa(persoana.get_adresa())
    
        persoana_noua = self.cauta_dupa_id(persoana.get_id_persoana())
        persoana_noua.set_nume(persoana.get_nume())
        persoana_noua.set_adresa(persoana.get_adresa())
